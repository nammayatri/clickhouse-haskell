...
fetchData (settings) fetches = do
  ...
  e <- Control.Exception.try $ do
    case settings of
      SingleHttp http@(HttpConnection _ mng _) -> do
        url <- genURL http queryWithType
        req <- parseRequest url
        ans <- responseBody <$> httpLbs req mng
        return $ LBS.toStrict ans
      HttpPool pool ->
        withResource pool $ \conn@(HttpConnection _ mng _) -> do
...
defaultHttpPool numStripes idleTime maxResources =
  liftIO $ createHttpPool def False numStripes idleTime maxResources >>= setupEnv

httpClient :: (MonadIO m) => String -> String -> Int -> String -> Bool -> m (Env HttpConnection w)
httpClient user password port host needTls = liftIO $ httpConnect user password port host needTls >>= setupEnv
